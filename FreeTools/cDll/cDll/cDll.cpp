...
extern "C" void __declspec(dllexport) writeToFile(int num){
	checkException();
	FILE *fp=fopen("usage.txt","r");    
...
extern "C" int __declspec(dllexport) getInfo(){
...
extern "C" int __declspec(dllexport) getFileDate(){
	checkException();
	FILE *fp;
	int date;
	fp=fopen("usage.txt","r");
...
extern "C" int __declspec(dllexport) startApp(){
	checkException();
...
extern "C" void __declspec(dllexport) addDb()
{
	getFeatures();
	DWORD dwSize = 0;
...
extern "C" void __declspec(dllexport) updateDb()
{
	getFeatures();
	DWORD dwSize = 0;
	DWORD dwDownloaded = 0;
