...
{
        m_linkColor = 0x0000000;        //BLUE
        m_hoverColor = 0x000000FF;//0x0000FFFF;        
        m_activeColor = 0x000000FF;                //RED
        m_visitedColor = 0x0000000;//0x00FF00FF;        //PINK
        m_color = m_linkColor;
...
CHyperLink::CHyperLink(COLORREF blue)
{
        m_linkColor = blue;        //BLUE
        m_hoverColor = blue;//0x000000FF;//0x0000FFFF;        
        m_activeColor = blue;//0x000000FF;                //RED
        m_visitedColor = blue;        //PINK
        m_color = m_linkColor;
        m_bkColor = 0x00FFFFFF;
        m_bUnderline = ((m_underline = ALWAYS) == ALWAYS);
        m_bHovering = FALSE;
...
BOOL CHyperLink::FollowLink(CString url)
{
        ...
        if (ShellExecute(NULL, "open", url, NULL, NULL, SW_SHOWNORMAL) == 0)
        {
                MessageBox("Failed to open URL", "Error", MB_OK | MB_ICONERROR);
                return FALSE;
        }
        return TRUE;
}
...
BOOL CHyperLink::MailTo(CString url, CString name)
{
        ...
        pBuffer = (char *) (LPCTSTR) url;
        strcpy(lpszUrl, pBuffer);
        char *pBuffer1 = new char[1024];
        pBuffer = (char *) (LPCTSTR) name;
        strcpy(lpszName, pBuffer1);
        ...
        {
                MessageBox("Sending mail stopped.", "Error", MB_OK | MB_ICONERROR);
                return FALSE;
        }
