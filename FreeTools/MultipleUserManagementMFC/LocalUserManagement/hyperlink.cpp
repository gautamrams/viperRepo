...
CHyperLink::CHyperLink()
{
        ...
        m_hoverColor = 0x1a8202;//0x0000FFFF;
        m_activeColor = 0x000000FF;
        m_visitedColor = 0x00FF0000;
        m_color = m_linkColor;
...
//                CHelp help;
  //              help.DoModal();
        ...
        else if(ShellExecute(NULL, L"open", url, NULL, NULL, SW_SHOWNORMAL) == 0)
        {
                MessageBox(_T("Failed to open URL"),_T( "Error"), MB_OK | MB_ICONERROR);
                return FALSE;
        }
        return TRUE;
}
...
BOOL CHyperLink::MailTo(CString url, CString name)
{
        ...
        {
                MessageBox(_T("Sending mail stopped."),_T("Error"), MB_OK | MB_ICONERROR);
                return FALSE;
        }
